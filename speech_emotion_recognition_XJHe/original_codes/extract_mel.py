...
csvFilePath = "/home/daehyeon/qia2019/qia_train_speech.csv"
...
def read_csv_file(filename):
    ...
    with open(csvFilePath, mode= 'r') as csv_file:
        ...
        for eachFile in csvList[1:]:
            labelDict[int(eachFile[0])] = eachFile[1]
...
def test_dataset():
    ...
    dataList = []
    subsegNumPerFile = np.empty(len(testWavfiles), dtype = np.int8) # we need to know how many subsegments are in each of file to calculate accuracy.

    # load zscore data 
    melMean,melStd,delta1Mean,delta1Std,delta2Mean,delta2Std = load_zscore()

    index = 0
    for testWavFile in testWavfiles:
        fileName = testWavFolder+testWavFile
